...
```mermaid
flowchart LR
  A[클라이언트 SET] --> B[Redis 메모리 반영]
  B --> C{영속성 정책}
  C --> D[RDB 스냅샷 시점 저장]
  C --> E[AOF 로그 기록]
  B -.장애 발생 가능 구간.-> F[유실 위험]
```

핵심 해석:
- 명령 성공 직후에는 메모리 반영이 먼저 일어납니다.
- 디스크 반영 전 장애가 나면 유실 가능성이 생깁니다.
...
## 초보자 체크
- "메모리 반영"과 "디스크 반영" 차이를 설명할 수 있는가?
- RDB와 AOF를 각각 언제 우선 선택할지 말할 수 있는가?
