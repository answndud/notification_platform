...
MGET user:{1001}:profile user:{1001}:token   -> 같은 슬롯, 가능
MGET user:1001:profile user:2002:token       -> 다른 슬롯일 수 있음, CROSSSLOT 가능
```

핵심 해석:
- Cluster는 "노드에 직접 저장"이 아니라 "슬롯을 통해 노드에 배치"됩니다.
- 다중 키 연산이 필요하면 해시 태그로 슬롯 공존을 의도적으로 설계해야 합니다.
...
- 왜 "노드 수"보다 "슬롯 분포"가 더 중요한지 설명할 수 있는가?
- 해시 태그를 언제 쓰고, 언제 남용하면 안 되는지 말할 수 있는가?
