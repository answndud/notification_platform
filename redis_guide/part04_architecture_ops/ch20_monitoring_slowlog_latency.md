...
```text
증상(사용자 체감) -> 원인 후보(인프라/명령/키) -> 검증(지표/재현)
...
```mermaid
flowchart TD
  A[API p95 상승 감지] --> B[Redis 리소스 지표 확인]
  B --> C[SLOWLOG/LATENCY 확인]
  C --> D[핫키/BigKey/명령 패턴 점검]
  D --> E[개선안 적용]
  E --> F[p95/p99 재측정]
```
...
- 평균 응답시간만 보면 왜 문제가 가려질 수 있는가?
- `SLOWLOG`와 `LATENCY`를 함께 봐야 하는 이유를 설명할 수 있는가?
