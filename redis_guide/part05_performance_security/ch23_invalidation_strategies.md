...
```text
쓰기 발생 -> 어떤 캐시를 지울지 결정 -> 삭제/갱신 실행 -> 실패 시 재시도

TTL 기반: 시간이 지나면 자연 만료
이벤트 기반: 변경 이벤트 시 즉시 무효화
```

```mermaid
flowchart TD
  A[DB 쓰기/도메인 이벤트] --> B[무효화 대상 키 계산]
  B --> C[DEL/UNLINK 또는 버전 증가]
  C --> D{성공?}
  D -- 예 --> E[정상 종료]
  D -- 아니오 --> F[재시도 큐/보정 배치]
```
...
핵심 해석:
- 무효화는 "키를 지우는 기술"이 아니라 "정합성 계약 실행"입니다.
- 실패 경로(재시도/보정)가 설계되어야 운영에서 안전합니다.
...
- TTL만으로 해결 가능한 도메인과 어려운 도메인을 구분할 수 있는가?
- 무효화 실패 시 왜 재시도 큐가 필요한지 설명할 수 있는가?
