...
```text
순수 데이터 -> 구조 오버헤드 -> 복제 반영 -> 운영 여유율

최종 필요 용량 = ((키+값 총량) * 오버헤드 계수) * 복제 배수 * 여유율
```
...
핵심 해석:
- 평균치만 보면 피크 구간 위험을 놓칩니다.
- 계산값은 출발점이고, 실제 운영 지표로 계속 보정해야 합니다.
...
1. `순수 데이터` = 평균 키+값 크기 * 키 개수
2. `구조 오버헤드` = 순수 데이터 * 오버헤드 계수(예: 1.2~1.8)
3. `복제 반영` = 위 결과 * 복제 배수
4. `운영 여유` = 위 결과 * 여유율(20~30%)

최종 용량은 위 계산값과 실측 메모리(`INFO memory`)를 비교해 보정합니다.
...
- 왜 복제 배수와 여유율을 누락하면 실제 비용이 크게 틀어지는지 설명할 수 있는가?
- 용량 계획을 일회성 작업이 아닌 지속 보정으로 봐야 하는 이유를 말할 수 있는가?
