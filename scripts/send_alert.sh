...
lines = [
    ...
    f"reasonDetails={','.join(data.get('reasonDetails', [])) if isinstance(data.get('reasonDetails'), list) else data.get('reasonDetails')}",
    f"requestQueuedLag={data.get('requestQueuedLag')}",
    f"malformedQueuedLag={data.get('malformedQueuedLag')}",
